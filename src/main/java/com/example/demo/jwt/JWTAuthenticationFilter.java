...
public class JWTAuthenticationFilter extends UsernamePasswordAuthenticationFilter {
    ...
    protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response, FilterChain chain, Authentication authResult) throws IOException, ServletException {
        User userDetail = (User) authResult.getAuthorities();
        // 从userDetail中获取权限信息
        Collection<? extends GrantedAuthority> authorities = userDetail.getAuthorities();
        //创建token
        String token = JwtTokenUtils.createToken(userDetail.getUsername(), authorities.toString());
        response.setHeader("token", JwtTokenUtils.TOKEN_PREFIX +token);
        ResponseUtil.responseJson(response,200, TokenVo.builder().token(token).build());
    }
